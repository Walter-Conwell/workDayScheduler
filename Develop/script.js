$(document).ready(function () {
  ...
  for (var i = 0; i < timeSlot.length; i++) {
    var timeList = Number(hrSlot[i].id);

    if (timeList < timeSlot) {
      hrSlot[i].classList.add("past");
    } else if (timeList === timeSlot) {
      hrSlot[i].classList.add("present");
    } else {
      hrSlot[i].classList.add("future");
    }
  }
